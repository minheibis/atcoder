...
@lru_cache
def count_route(start, end, move_time, edge_list, x, mod2):
    # 記録
    print("start, end, move_time, x, mod2: ", start, end, move_time, x, mod2)
    # 終了条件
    if move_time == 0:
        if start == end and mod2 == 0:
            return Modint(1)
        else:
            return Modint(0)
    # 再帰の場合
    ...
    for next_end in end_set:
        if next_end == x:
            print("next_end, change mod: ", next_end)
            mod2_new = mod2 + 1
            mod2_new %= 2
            count += count_route(start, next_end, move_time - 1, edge_list, x, mod2_new)
        else:
            count += count_route(start, next_end, move_time - 1, edge_list, x, mod2)
    return count
...
count = count_route(S, T, K, edge_list, x, mod2)
print(count)